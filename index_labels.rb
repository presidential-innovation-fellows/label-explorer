...
x_registrations.each_with_index do |reg,index| #iterate over links to label detail pages 
			...
			if index > 1 and index < 5
				...
				if registration_detail.css('table tr td .u-tL a')[0].attribute('href') 
					...
					encoded_string = Base64.encode64(File.open(local_path).read)#read in label PDF and encode in base 64
					client = Elasticsearch::Client.new log: true #start up new elasticsearch ruby client
